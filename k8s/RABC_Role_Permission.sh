************************************
User Creation with tenant admin access
*************************************************
--robin user list <username>
--robin tenant list <tenant name>
--robin namespace list <ns name>
--robin ldap list-servers to check AD servers list in the server.
robin user add  yatin.agrawal-a  tn-rcp-ros   tenantadmin --ldap --ldap-server kiba_ad 
robin user add svc.lcmtest tn-ie-ros-bmaas tenantadmin --ldap --ldap-server kiba_ad
 robin user add mon-kpi Administrators superadmin
 robin user change -tenant  tn-rm-rsp
 robin user change-tenant  tn-rm-rsp
 ********************************************
If user is already present in the tenant and need to add into another tenant below is the command.
******************************************************************************************************
robin tenant add-users omnicare santosh.akavarapu-a tenantadmin

**********************************************

Removing users:-
*************************
robin tenant remove-users tn_Mavenir_B2B-BSS-apps_staging2 arjun.manjunath-a
robin user -h
robin user remove arjun.manjunath-a 
******************************************************
Multipule users and single tenant:-
***********************************
1. Put user list in user-list.txt1 file
2.  for u in `cat user-list.txt`; do robin user add $u   tn-rcp-ros  tenantadmin --ldap --ldap-server kiba_ad; done
3. for u in `cat user-list.txt`; do robin user remove $u; done
multiple tenants and multiple users:-
**************************************

3. Put tenant list in tenant-list2.txt file
4. for t in `cat tenant-list5.txt`; do for u in `cat user-list.txt`; do robin tenant add-users $t $u tenantadmin; done; done

For sharing ns to user 
**************************
robin namespace share  ns-rakuten-b2b-mno-sandbox  bama.dutta-a      --force 
robin namespace list-shares | grep t074-u000258
for i in `cat namespace.txt`; do robin namespace share $i raghavendra.v-a; done
robin  namespace add 
*************************************For loop for creation of users************************
for i in `cat user-list.txt`; do robin namespace share ns-rcp-ros $i; done
for i in `cat user-list.txt`; do robin tenant add-users tn-rcp-ros $i tenantadmin; done
for i in `cat user-list.txt`; do robin namespace share ns-rcp-ros $i; done
for i in `cat user-list.txt`; do robin user list $i ; done

***************which namesspace are associated with tenant*****************
robin namespace list --tenant  tn_Mavenir_B2B-BSS-apps_staging3 --all-records

***********************************************For checking user is having resource permission(RBAC) or not:-******************************************************
kubectl auth can-i create vs --as rachit.soni-a -n  ns-rcp-ros
note:
vs--virtual service 
n-- namespace
...
metadata:
  ...
  name: RoleBinding-vinod-venkatraman
...
Note:
1.By default if user have RABC permission he can't do kubectl get top pods or nodes if he have certain limit permission then   you need to create above role with apiGroups: ["metrics.k8s.io"].
